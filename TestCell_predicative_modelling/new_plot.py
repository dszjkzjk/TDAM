...
"""
## Visulization Automation
def plot(hi_hihi,df,start_date,end_date,parameter):
    col_rename(df)
    ...
    # Rename col
        ...
        if hi != 'Missing Hi or HIHI' and hi != "Not in HI file":
            ...
            plt = ggplot(df_sub, aes(x='date', y=parameter, color='status')) + geom_point(size=50) + scale_x_date(labels='%Y-%m-%d') +\
            labs(title = "Test Cell 22 " + str(parameter) + " from " + str(start_date) + " to " + str(end_date)) + \
            geom_hline(y=hi[0],color='red') + geom_hline(y=hi[1],color='blue')
            return plt
        else:
            #df_sub['date'] = [dt.datetime.strptime(d,'%Y-%m-%d') for d in df_sub['Date']]
            plt = ggplot(df_sub, aes(x='date', y=parameter))+ geom_point(size=50) + scale_x_date(labels='%Y-%m-%d') + \
            labs(title = "Test Cell 22 " + str(parameter) + " from " + str(start_date) + " to " + str(end_date))
            return plt
...
def plot(tc_num,start_date,end_date,parameter):
    ...
    hihi = pd.read_csv('/Users/junkangzhang/Downloads/MMA 2019/Courses/BUSA693/Capstone/Individual/Individual 3/tc'+str(tc_num)+'/hihi.csv')
    col_rename(df)
...
#df = pd.read_csv('/Users/junkangzhang/Downloads/MMA 2019/Courses/BUSA693/Capstone/Individual/Individual 2/cleaned_22_data(1).csv')
#hi_hihi = pd.read_excel('/Users/junkangzhang/Downloads/MMA 2019/Courses/BUSA693/Capstone/Individual/Individual 2/sub/hi_hihi.xlsx')
plot(22,'2016-01-01','2018-12-31','DYNOBRG')
